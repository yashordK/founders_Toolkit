#!/usr/bin/env node
// Pulls recall() context on a topic for brainstorm mode to think with.
// Verified via agent/verify/verify_recall.mjs before being handed back, same
// as warm-intro/daily-plan/nudge-sweep -- see openclaw-skills/warm-intro/
// SKILL.md for why that check exists.
//
// Usage:
//   node fetch_context.mjs --topic "pricing strategy for enterprise"
//
// Env overrides:
...
async function main() {
  ...
  if (!topic) {
    console.error('usage: fetch_context.mjs --topic "<what to brainstorm about>"');
    process.exit(1);
  }
